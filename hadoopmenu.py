...
login=input("Local [L] , Remote [R] , AWS Instance [I]: ")
if login=="L":
	while True :
	#r=sr.Recognizer()
	#with sr.Microphone() as source:
		...
		#data = r.recognize_google(audio)
			...
			if int(data)==1:
				...
				down=input("Do you want to download Softwares of Hadoop? [Y] / [N]: ")
				if down=="Y":
					os.system("scp -r jdk-8u171-linux-x64.rpm root@{}:/".format(ipremote))
					os.system("scp -r hadoop-1.2.1-1.x86_64.rpm root@{}:/".format(ipremote))
				else:
					print("ok Processing")
				os.system("scp -r hadooporgrem.py root@{}:/".format(ipremote))
				os.system(r"ssh root@{} python3 /hadooporgrem.py".format(ipremote))
elif login=="I":
			ip = input("Enter Ip Address: ")
			down=input("Do you want to download Softwares of Hadoop? [Y] / [N]: ")
			if down=="Y":
				os.system("scp -i hadoopkey.pem -r jdk-8u171-linux-x64.rpm ec2-user@{}:~".format(ip))
				os.system("scp -i hadoopkey.pem -r hadoop-1.2.1-1.x86_64.rpm ec2-user@{}:~".format(ip))
			else:
				print("ok Processing")
			os.system("scp -i hadoopkey.pem -r hadooporgrem.py ec2-user@{}:~".format(ip))
			os.system("ssh -i hadoopkey.pem -l ec2-user {} sudo python3 hadooporgrem.py".format(ip))
